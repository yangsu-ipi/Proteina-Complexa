...
environment (`preflight.sh:58-61`), and the fallback found nothing exported.
...
1. `validate_env` tested for the `.env` **file** in cwd and returned early, never
   ...
   only a genuinely unset `DATA_PATH` fails. Same fall-through `preflight.sh:58-61` uses.
...
reads `os.environ` (`validate.py:137-152`), and `load_dotenv` defaults to
...
(`configs/pipeline/binder/binder_evaluate.yaml:101`).
...
load location ahead of the HF cache (`:598-599`), and then `from_pretrained(...,
local_files_only=True)` fails. With `force_offline=True` — the default on
`compute_esm_ppl_for_sequences` (`:887-890`) — you get `RuntimeError: ESM model not found in
...
`_resolve_esm_dir()` returns `None` (`evaluation/esm_eval.py:528-544`) and the code falls
