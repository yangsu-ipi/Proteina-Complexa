...
(`evaluation/esm_eval.py:461-466`), so an empty directory resolves, becomes the *first*
load location ahead of the HF cache (`:519-520`), and then `from_pretrained(...,
local_files_only=True)` fails. With `force_offline=True` — the default on
`compute_esm_ppl_for_sequences` (`:743`) — you get `RuntimeError: ESM model not found in
...
`_resolve_esm_dir()` returns `None` (`evaluation/esm_eval.py:451-467`) and the code falls
