...
- Exported as `COMPLEXA_INIT="<runtime>"` by the generated `env.sh` (`cli_runner.py:1702`). It appears in no `.env` file.
- **This is the gate on the whole CLI.** `_check_complexa_init` (`cli_runner.py:1999-2014`) exits 1 with "Environment not initialized" for `design`, `generate`, `filter`, `evaluate`, `analyze`, `analysis`, and `target` when it is unset. `init`, `demo`, `download`, `validate`, and `status` are exempt.
...
1. `.env` file exists **in the current working directory** — `validate.py:261` is a bare `Path(".env")`, with no parent walk. (`complexa init` *does* walk parents, but only to find `.env_example`.)
