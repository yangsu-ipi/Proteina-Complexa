...
> it. Generation honours the injected `root_path` (`generate.py:695, :564-574`), but
...
The combined CSV has **one row per generated sample** (`id_gen`, an enumerate index — `binder_eval.py:289, :216`), with one column *prefix* per requested `metric.sequence_types` value:
...
| `{seq}_sequence` | the binder sequence (`binder_eval.py:425`) |
