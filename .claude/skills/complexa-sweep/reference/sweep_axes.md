...
| `success_rate` | **Read it from `filter_results/res_filter_binder_pass_*.csv`.** | Recompute only if that file is absent, and then use all **six** defaults from `DEFAULT_PROTEIN_BINDER_THRESHOLDS` (`binder_analysis_utils.py:98-183`): `{seq}_complex_i_pAE_all * 31 <= 7.0` AND `{seq}_complex_pLDDT_all >= 0.9` AND `{seq}_binder_scRMSD_ca_all < 1.5` AND `{seq}_apo_scRMSD_ca_<model>_all < 2.0` AND `{seq}_complex_scRMSD_ca_all < 2.0` AND `{seq}_binder_scRMSD_target_aligned_ca_all < 2.0`. Using only the first three overreports — measured, 97 designs against 89. There is **no** `passes_filter` column — repo-wide grep matches only this skill's files. |
