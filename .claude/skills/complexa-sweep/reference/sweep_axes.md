...
|---|---|---|---|---|
| `metric.folding_models` | `[af2, esmfold2]` | any list of `af2`, `esmfold2`, `esmfold`, `rf3` | varies | Every folder the campaign refolds with, for every track. A member that cannot serve a track is warned about and skipped for that track (`esmfold` folds no complex; `rf3` folds no isolated monomer); an unrecognised name is refused outright, because the monomer gate would otherwise silently loosen. Sweeping this sweeps COST as well as coverage: each added folder refolds the complex, the apo monomer, designability and codesignability. |
...
| `metric.pre_refolding.{bioinformatics,tmol}` | both `false` (`binder_evaluate.yaml:131-133`) | `true`/`false` | minor | Toggle individual pre-refold interface metric modules. Only these two exist — `evaluate.py:405-407` reads `bioinformatics` and `tmol` and nothing else, and there is no `hbplus` module anywhere in `src/proteinfoundation/rewards/`. `metric.refolded.{bioinformatics,tmol}` are the post-refold twins and are also both `false` (`:139-141`). |
...
| `success_rate` | **Read it from `filter_results/res_filter_binder_pass_*.csv`.** | Recompute only if that file is absent, and then use all **six** defaults from `DEFAULT_PROTEIN_BINDER_THRESHOLDS` (`binder_analysis_utils.py:98-183`): `{seq}_complex_{backend}_i_pAE_all * 31 <= 7.0` AND `{seq}_complex_{backend}_binder_pLDDT_all >= 0.9` AND `{seq}_complex_{backend}_binder_scRMSD_ca_all < 1.5` AND `{seq}_apo_{model}_binder_scRMSD_ca_all < 2.0` AND `{seq}_complex_{backend}_scRMSD_ca_all < 2.0` AND `{seq}_complex_{backend}_binder_scRMSD_target_aligned_ca_all < 2.0`. Using only the first three overreports — measured, 97 designs against 89. There is **no** `passes_filter` column — repo-wide grep matches only this skill's files. |
...
list columns that the threshold filter reads (`binder_analysis_utils.py:249-273`).
