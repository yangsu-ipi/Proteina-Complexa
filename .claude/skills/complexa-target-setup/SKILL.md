...
| `missing checkpoint` but the file exists; path ends `checkpoints/` | `LOCAL_CHECKPOINT_PATH` defaults to `checkpoints/` (`.env_example:28`); downloaders write `ckpts/` (`download_startup.sh:239`) — usually gate-only if the pipeline YAML sets `ckpt_path` absolutely | `LOCAL_CHECKPOINT_PATH=${LOCAL_CODE_PATH}/ckpts`, then re-init |
