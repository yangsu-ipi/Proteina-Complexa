...
| Ligand binder (binder + small-molecule) | Same config, no overrides needed | `configs/evaluate_from_pdb_dir.yaml` | `ligand_binder` (shipped default, `:152`) | `rf3_latest` (`:72`) |
...
`result_type: ligand_binder` (`:152`), `aggregation.analysis_modes: [binder]` (`:159`). Omit the
...
- **Primary CSV** from `analyze` — `RAW_{result_type}_results_{config_name}_combined.csv`
  ...
  enumerate index over the file walk (`binder_eval.py:332, :259`). `sequence_types` are column
  **prefixes** on that single row, not extra rows — `self_complex_i_pAE`,
  ...
  lists (`binder_eval.py:431-467`; `binder_analysis_utils.py:160-171` builds
