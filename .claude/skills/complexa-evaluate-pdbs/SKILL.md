...
| Ligand binder (binder + small-molecule) | Same config, no overrides needed | `configs/evaluate_from_pdb_dir.yaml` | `ligand_binder` (shipped default, `:177`) | `rf3_latest` (`:72`) |
...
`result_type: ligand_binder` (`:177`), `aggregation.analysis_modes: [binder]` (`:184`). Omit the
