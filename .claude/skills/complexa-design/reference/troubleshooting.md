...
`colabdesign` or a name containing `rf3` (`binder_eval.py:121-141`), so
...
(`binder_eval.py:130-135`), so the evaluate stage cannot raise an
...
`success_thresholds` dict. `binder_analysis.py:460-461` uses
...
(`analysis_utils.py:132-137`) fills the missing `scale` with `1.0` so a
