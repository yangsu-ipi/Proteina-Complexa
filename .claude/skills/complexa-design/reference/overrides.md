...
| `metric.folding_models` | list | `[af2, esmfold2]` (binder), `[rf3, af2, esmfold2]` (ligand, AME) | `++metric.folding_models=[af2,esmfold2]` | Every folder this campaign refolds with, for EVERY track: the complex, the apo monomer, designability and codesignability. Each track gets the members that can serve it — `af2` and `esmfold2` fold both a monomer and a complex, `esmfold` only a monomer, `rf3` only a complex — and a member that cannot serve a track is named in the run log rather than dropped in silence. The first complex-capable member is the one the complex thresholds are written against. Replaces `binder_folding_method`, `monomer_folding_models`, `designability_folding_models`, `codesignability_folding_models`, `apo_folding_models` and `consensus_backends`; setting any of those alongside it is refused when they disagree. A ligand target drops `af2` from the complex track, since ColabDesign refuses one. |
...
| ~~`metric.monomer_folding_models`~~ | — | — | — | Retired: use `metric.folding_models`, which covers the monomer tracks and the complex alike |
...
| `aggregation.designability_thresholds.ca.{af2,esmfold2}.threshold` | float | `2.0` | `++aggregation.designability_thresholds.ca.{af2,esmfold2}.threshold=1.5` | Designability CA-scRMSD threshold |
| `aggregation.ca_codesignability_thresholds.ca.{af2,esmfold2}.threshold` | float | `2.0` | `++aggregation.ca_codesignability_thresholds.ca.{af2,esmfold2}.threshold=1.5` | CA codesignability threshold |
| `aggregation.allatom_codesignability_thresholds.all_atom.{af2,esmfold2}.threshold` | float | `2.0` | `++aggregation.allatom_codesignability_thresholds.all_atom.{af2,esmfold2}.threshold=2.5` | All-atom codesignability threshold |
