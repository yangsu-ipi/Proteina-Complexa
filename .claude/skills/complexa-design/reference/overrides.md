...
| `metric.interface_cutoff` | float | `8.0` (protein target) / `6.0` (ligand target) — code defaults `DEFAULT_INTERFACE_CUTOFF_PROTEIN` / `_LIGAND` (`binder_eval_utils.py:50-51`); `ame_evaluate.yaml` does not set the key at all | `++metric.interface_cutoff=6.0` | Interface-residue distance cutoff in Angstroms |
