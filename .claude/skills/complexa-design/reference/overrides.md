...
| `metric.binder_folding_method` | enum | `colabdesign` (binder), `rf3_latest` (ligand, AME) | `++metric.binder_folding_method=rf3_latest` | Refold backend. Only `colabdesign` or a name containing `rf3` (e.g. `rf3_latest`) is accepted; anything else raises `ValueError` (`binder_eval.py:101-121`). `colabdesign` is the only AF2 path, and it rejects ligand targets. `esmfold` belongs to the *different* key `metric.monomer_folding_models`. |
