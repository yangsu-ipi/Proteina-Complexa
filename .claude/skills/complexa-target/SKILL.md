...
The name must be new. `02_PDL1` and friends already exist in `configs/targets/targets_dict.yaml`, and without `-f` a collision makes `target_manager.py:1035-1044` prompt on `input()` — which raises `EOFError` non-interactively, cancels the add, and exits 1 (`cli_runner.py:1907-1908`).
...
| `⚠️  Target 'X' already exists!` then `Overwrite? (y/N):` | Name collision in the dict being written (`target_manager.py:1035-1044`) | Pick a new name, or pass `-f / --force`. Non-interactively the `input()` raises `EOFError`, the add is cancelled and `cli_runner.py:1907-1908` exits 1 — so always check the name first. |
