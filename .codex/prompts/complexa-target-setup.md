...
| `docs/binder-target-setup/env-and-mirrors.md` | mirror env vars, layouts, build recipes |
...
(`cli_runner.py:1981` tests presence only). Point `UV_VENV` and the `UV_*` tool vars at the
...
The `cd` matters: `./inference/...` (`generate.py:62`) and `./logs` (`cli_runner.py:128`)
