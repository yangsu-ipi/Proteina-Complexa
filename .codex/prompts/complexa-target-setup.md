...
(`cli_runner.py:2009` tests presence only). Point `UV_VENV` and the `UV_*` tool vars at the
...
The `cd` matters: `./inference/...` (`generate.py:68`) and `./logs` (`cli_runner.py:128`)
...
| `Environment not initialized. Run: complexa init` | `COMPLEXA_INIT` unset (`cli_runner.py:2002-2014`) | source `env.sh` from **bash** |
...
| `missing community model path: ESM_DIR` | ESM2 absent; not gate-only (`binder_evaluate.yaml:101`) | `complexa download --esm2`, or `++metric.compute_esm_metrics=false` |
